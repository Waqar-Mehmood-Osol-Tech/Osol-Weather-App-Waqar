...
export const get5DaysForecast = createAsyncThunk("weather/get5DaysForecast", async (obj) => {
  try {
    const request = await axios.get(
      `${hostName}/data/2.5/forecast?lat=${obj.lat}&lon=${obj.lon}&units=${obj.unit}&APPID=${appId}`
    );
...
// New thunk for adding a city to selected cities
export const addCityToFavorites = createAsyncThunk(
  "weather/addCityToFavorites",
  async (city, { getState, dispatch }) => {
    const state = getState();
    const existingCity = state.weather.selectedCities.find(c => c.city === city.city);
...
const weatherSlice = createSlice({
  ...
  reducers: {
    removeCity: (state, action) => {
      state.selectedCities = state.selectedCities.filter(city => city.city !== action.payload);
